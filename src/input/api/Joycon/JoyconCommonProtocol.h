...
public:
     ...
     */
    DriverResult SendRawData(std::span<const uint8> buffer);

    template <typename Output>
        requires std::is_trivially_copyable_v<Output>
    DriverResult SendData(const Output& output) {
        std::array<uint8, sizeof(Output)> buffer;
     ...
     */
    DriverResult SendSubCommand(SubCommand sc, std::span<const uint8> buffer,
     ...
     * @param buffer data to be send
     */
    DriverResult SendMCUCommand(SubCommand sc, std::span<const uint8> buffer);

    /**
     * Sends vibration data to the joycon
     * @param buffer data to be send
     */
    DriverResult SendVibrationReport(std::span<const uint8> buffer);
     ...
     * @returns output buffer containing the response
     */
    DriverResult ReadRawSPI(SpiAddress addr, std::span<uint8> output);
     ...
     * @returns output buffer containing the response
     */
    DriverResult SendMCUData(ReportMode report_mode, MCUSubCommand sc, std::span<const uint8> buffer,
     ...
     */
    uint8 CalculateMCU_CRC8(uint8* buffer, uint8 size) const;
