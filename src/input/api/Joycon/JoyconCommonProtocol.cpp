...
uint8 JoyconCommonProtocol::GetCounter() {
...
DriverResult JoyconCommonProtocol::SetReportMode(ReportMode report_mode) {
    const std::array<uint8, 1> buffer{static_cast<uint8>(report_mode)};
    return SendSubCommand(SubCommand::SET_REPORT_MODE, buffer);
}

DriverResult JoyconCommonProtocol::SendRawData(std::span<const uint8> buffer) {
...
DriverResult JoyconCommonProtocol::GetSubCommandResponse(SubCommand sc,
    ...
    do {
        int result = SDL_hid_read_timeout(hidapi_handle->handle, reinterpret_cast<uint8*>(&output),
...
DriverResult JoyconCommonProtocol::SendSubCommand(SubCommand sc, std::span<const uint8> buffer,
                                                  SubCommandResponse& output) {
...
DriverResult JoyconCommonProtocol::SendMCUCommand(SubCommand sc, std::span<const uint8> buffer) {
    SubCommandPacket packet{
...
DriverResult JoyconCommonProtocol::SendVibrationReport(std::span<const uint8> buffer) {
...
DriverResult JoyconCommonProtocol::ReadRawSPI(SpiAddress addr, std::span<uint8> output) {
    constexpr std::size_t HeaderSize = 5;
    constexpr std::size_t MaxTries = 10;
    std::size_t tries = 0;
    SubCommandResponse response{};
    std::array<uint8, sizeof(ReadSpiPacket)> buffer{};
    const ReadSpiPacket packet_data{
        .spi_address = addr,
        .size = static_cast<uint8>(output.size()),
    };
...
DriverResult JoyconCommonProtocol::EnableMCU(bool enable) {
    const std::array<uint8, 1> mcu_state{static_cast<uint8>(enable ? 1 : 0)};
...
DriverResult JoyconCommonProtocol::ConfigureMCU(const MCUConfig& config) {
    std::array<uint8, sizeof(MCUConfig)> config_buffer;
    ...
    do {
        ...
        if (tries++ > MaxTries) {
            return DriverResult::WrongReply;
             ...
             output.mcu_data[6] != static_cast<uint8>(mode));

    return DriverResult::Success;
}

// crc-8-ccitt / polynomial 0x07 look up table
constexpr std::array<uint8, 256> mcu_crc8_table = {
...
uint8 JoyconCommonProtocol::CalculateMCU_CRC8(uint8* buffer, uint8 size) const {
    uint8 crc8 = 0x0;

    for (int i = 0; i < size; ++i) {
        crc8 = mcu_crc8_table[static_cast<uint8>(crc8 ^ buffer[i])];
    }
    return crc8;
}
