...
DriverResult NfcProtocol::ReadAmiibo(std::vector<uint8>& data) {
    ScopedSetBlocking sb(this);
    DriverResult result{DriverResult::Success};
    TagFoundData tag_data{};
...
DriverResult NfcProtocol::WriteAmiibo(std::span<const uint8> data) {
    ScopedSetBlocking sb(this);
    DriverResult result{DriverResult::Success};
    ...
    do {
        ...
        if (tries++ > timeout_limit) {
            return DriverResult::Timeout;
             ...
             output.mcu_data[5] != 0x31 || output.mcu_data[6] != static_cast<uint8>(status));
...
DriverResult NfcProtocol::IsTagInRange(TagFoundData& data, std::size_t timeout_limit) {
    ...
    data.uuid_size = std::min(output.mcu_data[14], static_cast<uint8>(sizeof(TagUUID)));
    memcpy(data.uuid.data(), output.mcu_data.data() + 15, data.uuid.size());

    return DriverResult::Success;
}

DriverResult NfcProtocol::GetAmiiboData(std::vector<uint8>& ntag_data) {
...
DriverResult NfcProtocol::WriteAmiiboData(const TagUUID& tag_uuid, std::span<const uint8> data) {
    constexpr std::size_t timeout_limit = 60;
    const auto nfc_data = MakeAmiiboWritePackage(tag_uuid, data);
    const std::vector<uint8> nfc_buffer_data = SerializeWritePackage(nfc_data);
    std::span<const uint8> buffer(nfc_buffer_data);
    MCUCommandResponse output{};
    uint8 block_id = 1;
    uint8 package_index = 0;
    std::size_t tries = 0;
...
DriverResult NfcProtocol::GetMifareData(const MifareUUID& tag_uuid,
    ...
    const std::vector<uint8> nfc_buffer_data = SerializeMifareReadPackage(nfc_data);
    std::span<const uint8> buffer(nfc_buffer_data);
    DriverResult result = DriverResult::Success;
    MCUCommandResponse output{};
    uint8 block_id = 1;
    ...
    while (tries++ < timeout_limit) {
        ...
        if (output.mcu_report == MCUReport::NFCState && output.mcu_data[1] == 0x10) {
            ...
            const uint8 number_of_elements = output.mcu_data[10];
...
DriverResult NfcProtocol::WriteMifareData(const MifareUUID& tag_uuid,
    ...
    const std::vector<uint8> nfc_buffer_data = SerializeMifareWritePackage(nfc_data);
    std::span<const uint8> buffer(nfc_buffer_data);
    DriverResult result = DriverResult::Success;
    MCUCommandResponse output{};
    uint8 block_id = 1;
...
DriverResult NfcProtocol::SendStartPollingRequest(MCUCommandResponse& output,
    ...
    NFCRequestState request{
        ...
        .nfc_polling =
            {
                .enable_mifare = 0x00,
                .unknown_1 = static_cast<uint8>(is_second_attempt ? 0xe8 : 0x00),
                .unknown_2 = static_cast<uint8>(is_second_attempt ? 0x03 : 0x00),
                .unknown_3 = 0x2c,
                .unknown_4 = 0x01,
            },
        .crc = {},
    ...
    std::array<uint8, sizeof(NFCRequestState)> request_data{};
...
DriverResult NfcProtocol::SendNextPackageRequest(MCUCommandResponse& output, uint8 packet_id) {
    NFCRequestState request{
    ...
    std::vector<uint8> request_data(sizeof(NFCRequestState));
    memcpy(request_data.data(), &request, sizeof(NFCRequestState));
    request_data[36] = CalculateMCU_CRC8(request_data.data(), 36);
    return SendMCUData(ReportMode::NFC_IR_MODE_60HZ, MCUSubCommand::ReadDeviceMode, request_data,
                       output);
...
DriverResult NfcProtocol::SendWriteDataAmiiboRequest(MCUCommandResponse& output, uint8 block_id,
                                                     bool is_last_packet,
    ...
    NFCRequestState request{
        ...
        .data_length = static_cast<uint8>(data_size),
        .raw_data = {},
        .crc = {},
    };
...
DriverResult NfcProtocol::SendReadDataMifareRequest(MCUCommandResponse& output, uint8 block_id,
...
std::vector<uint8> NfcProtocol::SerializeWritePackage(const NFCWritePackage& package) const {
    const std::size_t header_size =
        sizeof(NFCWriteCommandData) + sizeof(NFCWritePackage::number_of_chunks);
    std::vector<uint8> serialized_data(header_size);
...
std::vector<uint8> NfcProtocol::SerializeMifareReadPackage(const MifareReadPackage& package) const {
...
std::vector<uint8> NfcProtocol::SerializeMifareWritePackage(const MifareWritePackage& package) const {
    const std::size_t header_size = sizeof(MifareCommandData);
    std::vector<uint8> serialized_data(header_size);
    std::size_t start_index = 0;
...
NFCWritePackage NfcProtocol::MakeAmiiboWritePackage(const TagUUID& tag_uuid,
                                                    std::span<const uint8> data) const {
    return {
        .command_data{
            .unknown = 0xd0,
            .uuid_length = sizeof(NFCReadCommandData::uid),
            ...
            .unknown8 = 0xff,
            .magic = data[16],
            .write_count = static_cast<uint16>((data[17] << 8) + data[18]),
...
MifareReadPackage NfcProtocol::MakeMifareReadPackage(
    ...
    MifareReadPackage package{
        .command_data{
            .unknown1 = 0xd0,
            .unknown2 = 0x07,
            .number_of_short_bytes = static_cast<uint8>(
...
NFCDataChunk NfcProtocol::MakeAmiiboChunk(uint8 page, uint8 size, std::span<const uint8> data) const {
    constexpr uint8 NFC_PAGE_SIZE = 4;
...
TagUUID NfcProtocol::GetTagUUID(std::span<const uint8> data) const {
    if (data.size() < 10) {
        return {};
    }
